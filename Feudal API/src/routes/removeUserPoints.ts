...
router.post('/:userId', async (req, res) => {
    try {
        ...
        await db.query(updateQuery, [params['userId'], queryResults.rows[0]['points'] - +json['points'] > 0 ? queryResults.rows[0]['points'] - +json['points'] : 0]);
        res.send(JSON.stringify({
            user_id: params['userId'],
            house: null,
            points: queryResults.rows[0]['points'] - +json['points'] > 0 ? queryResults.rows[0]['points'] - +json['points'] : 0,
            joined_at: null,
            success: true
        }));
    } catch(e) {
